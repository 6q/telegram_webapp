...
final class WeDevs_Dokan {
    ...
    public function scripts() {
		...
		if ( (is_singular( 'product' ) || is_product_category() || is_front_page()) && !get_query_var( 'edit' ) ) {
		/* Code end by aslam */
    ...
    function add_dashboard_template_class( $classes ) {
		...
		/* Code by aslam */
		global $post;
		//echo $post->ID; die('come');

        //if ( is_page( $page_id ) || ( get_query_var( 'edit' ) && is_singular( 'product' ) )) {
		if ( is_page( $page_id ) || ( get_query_var( 'edit' ) && is_singular( 'product' ) ) || ($post->ID == '7' || $post->ID == '2860')) {
